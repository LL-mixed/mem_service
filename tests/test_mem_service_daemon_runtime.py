...
class MemServiceDaemonRuntimeTests(unittest.TestCase):
    ...
    def _compile_typed_client_binary(self):
        ...
        source.write_text(
            ...
            "    };\n"
            "    struct mem_service_client_block_entry stale_prefix = prefix;\n"
            "    struct mem_service_client_block_entry conflicting_prefix = prefix;\n"
            "    struct mem_service_client_block_entry newer_prefix = prefix;\n"
            ...
            "    if (mem_service_client_materialize_object(&client, \"typed-object\", \"/tmp/mem-service-typed-object.materialized\", true, 11, true, 99, &record, &status) == 0 || status != MEM_SERVICE_WIRE_STATUS_UNSUPPORTED) return fail(\"materialize_object unsupported mismatch\");\n"
            ...
            "    stale_prefix.block_hash = \"typed-stale-block\";\n"
            "    stale_prefix.result_segment_id = 8191;\n"
            "    if (mem_service_client_register_prefix_entry(&client, &stale_prefix, &record, &status) == 0 || status != MEM_SERVICE_WIRE_STATUS_VERSION_CONFLICT) return fail(\"stale prefix publication was accepted\");\n"
            "    conflicting_prefix.block_hash = \"typed-conflicting-block\";\n"
            "    if (mem_service_client_register_prefix_entry(&client, &conflicting_prefix, &record, &status) == 0 || status != MEM_SERVICE_WIRE_STATUS_VERSION_CONFLICT) return fail(\"same-generation prefix conflict was accepted\");\n"
            "    if (expect_ok(mem_service_client_register_prefix_entry(&client, &prefix, &record, &status), status, \"register_prefix_retry\") != 0) return 1;\n"
            "    newer_prefix.block_hash = \"typed-newer-block\";\n"
            "    newer_prefix.result_segment_id = 8193;\n"
            "    if (expect_ok(mem_service_client_register_prefix_entry(&client, &newer_prefix, &record, &status), status, \"register_newer_prefix\") != 0) return 1;\n"
            "    if (expect_ok(mem_service_client_lookup_prefix_entry(&client, \"typed-request\", \"typed-prefix\", &record, &status), status, \"lookup_newer_prefix\") != 0) return 1;\n"
            "    if (strcmp(record.block_hash, \"typed-newer-block\") != 0 || record.last_result_segment != 8193) return fail(\"newer prefix publication mismatch\");\n"
    ...
    def _compile_provider_daemon_binary(self) -> Path:
        source = self.root / "mem_service_provider_daemon.c"
        binary = self.root / "mem_service_provider_daemon"
...
#include "components/mem_service/mem_service_provider.h"

static int fixture_probe(
    void *context,
    enum mem_service_provider_state *state_out)
{
    if (context == 0 || state_out == 0) {
        return -1;
    }
...
static int fixture_submit(
    void *context,
    const struct mem_service_transfer_request *request,
    uint64_t *completion_id_out)
{
    uint64_t *next_id = context;

    if (next_id == 0 || request == 0 || completion_id_out == 0) {
        return -1;
    }
    *next_id += 1;
    *completion_id_out = *next_id;
    return 0;
}

static int fixture_poll(
    void *context,
    ...
    struct mem_service_transfer_completion *completion_out)
{
    ...
    if (next_id == 0 || completion_out == 0 ||
        completion_id == 0 || completion_id != *next_id) {
        return -1;
    }
...
int main(int argc, char **argv)
{
    static const struct mem_service_provider_ops ops = {
        .probe = fixture_probe,
        .submit_transfer = fixture_submit,
        .poll_completion = fixture_poll,
    };
    ...
    registration.name = "fixture";
    registration.instance = "injected";
    registration.capabilities = MEM_SERVICE_PROVIDER_CAP_PEER_TRANSFER;
    registration.ops = &ops;
    registration.context = &next_id;
    ...
    return mem_service_run_unix_daemon_with_runtime(
        argv[1], 0, 0, 0, &runtime);
}
'''
        )
        cmd = [
...
'''
    ...
    def test_provider_registry_fixture_and_neutral_readiness(self):
        fixture = self._run_client("provider-fixtures")
        self.assertEqual(fixture.returncode, 0, fixture.stderr + fixture.stdout)
        self.assertIn("status=ok", fixture.stdout)
        self.assertIn("all_required=fail-closed", fixture.stdout)
        self.assertIn("bounds=fail-closed", fixture.stdout)
        self.assertIn("sdk=ok", fixture.stdout)
        self.assertIn("descriptor_wire=fail-closed", fixture.stdout)

        proc = self._start_server()
        try:
            status = self._run_client(
                "provider-status",
                "--connect",
                f"unix:{self.socket}",
            )
            self.assertEqual(status.returncode, 0, status.stderr + status.stdout)
            self.assertIn("control_plane_ready=1", status.stdout)
            self.assertIn("provider_registry_ready=1", status.stdout)
            self.assertIn("durable_ready=1", status.stdout)
            self.assertIn("data_plane_ready=0", status.stdout)
            self.assertIn("provider_count=0", status.stdout)
            self.assertIn("provider_ready_count=0", status.stdout)
            self.assertNotIn("shmem_ready=", status.stdout)
            self.assertNotIn("urma_ready=", status.stdout)
            self.assertNotIn("block_ready=", status.stdout)
        finally:
            self._stop_server(proc)

    def test_daemon_runtime_accepts_injected_ready_provider_registry(self):
        provider_daemon = self._compile_provider_daemon_binary()
        proc = subprocess.Popen(
            [str(provider_daemon), f"unix:{self.socket}"],
            cwd=REPO_ROOT,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            text=True,
        ...
        try:
            ...
            while time.time() < deadline:
                ...
                self.fail("provider daemon did not become ready")

            self.assertIn("data_plane_ready=1", status.stdout)
            self.assertIn("provider_count=1", status.stdout)
            self.assertIn("provider_ready_count=1", status.stdout)
        finally:
            self._stop_server(proc)
    ...
    def test_release_manifest_cli_matches_checked_in_contract(self):
        ...
        self.assertIn("operations=39", fixtures.stdout)
        self.assertIn("schema_manifest_len=17613", fixtures.stdout)
        self.assertIn("schema_manifest_checksum=0xb0964fb0", fixtures.stdout)
        self.assertIn("durable_backends=1", fixtures.stdout)
        self.assertIn("durable_catalogs=1", fixtures.stdout)
        self.assertIn("payload_block_backends=5", fixtures.stdout)
        ...
        self.assertIn("api_abi_policy_len=1025", fixtures.stdout)
        self.assertIn("api_abi_policy_checksum=0x5e460a87", fixtures.stdout)
        self.assertIn("admin_output_schema_len=7648", fixtures.stdout)
        self.assertIn("admin_output_schema_checksum=0xd4f33080", fixtures.stdout)
        self.assertIn("upgrade_rollback_policy_len=2144", fixtures.stdout)
        self.assertIn("upgrade_rollback_policy_checksum=0xfd2b541d", fixtures.stdout)
        ...
        self.assertIn("package_manifest_checksum=0x717b7ebb", fixtures.stdout)
        ...
        self.assertIn("compat_matrix_len=2226", fixtures.stdout)
        self.assertIn("compat_matrix_checksum=0xef0fb6d9", fixtures.stdout)
        self.assertIn("compat_baseline_len=1252", fixtures.stdout)
        self.assertIn("compat_baseline_checksum=0xa4c8146c", fixtures.stdout)
        self.assertIn("compat_old_new_matrix_len=1734", fixtures.stdout)
        self.assertIn("compat_old_new_matrix_checksum=0x6aef243d", fixtures.stdout)
    ...
    def test_version_cli_reports_release_contract(self):
        ...
        self.assertIn("wire_schema_manifest_checksum=0xb0964fb0", version.stdout)
        self.assertIn("api_abi_policy_checksum=0x5e460a87", version.stdout)
        self.assertIn("package_manifest_len=9814", version.stdout)
        self.assertIn("package_manifest_checksum=0x717b7ebb", version.stdout)
    ...
    def test_release_readiness_cli_reports_external_certification_blockers(self):
        ...
        self.assertIn("package_manifest_checksum=0x717b7ebb", readiness.stdout)
        self.assertIn(
    ...
    def test_release_readiness_cli_certifies_verified_evidence_files(self):
        ops_evidence = (
            ...
            "package_manifest_checksum=0x717b7ebb\n"
    ...
    def test_release_certification_verifier_reaches_readiness_gate(self):
        ...
        # Certification has a bounded request deadline. Build its prerequisite
        # explicitly so cold or sharded runs do not depend on earlier tests.
        build = subprocess.run(
            ["make", "-C", str(app_dir), "linqu_mem_service_host"],
    ...
    def test_package_manifest_cli_matches_checked_in_contract(self):
        ...
        self.assertIn("manifest_checksum=0x717b7ebb", fixtures.stdout)
        self.assertIn("installed_files=57", fixtures.stdout)
    ...
    def test_restore_policy_fixtures_fail_closed_until_commit(self):
        ...
        self.assertIn(
            "fail_closed_cases=bad-magic,future-store-schema,malformed-store-schema,"
            "out-of-order-page,record-count-mismatch,cancelled-stage-commit",
            fixtures.stdout,
        )
        self.assertIn("live_state=unchanged-until-commit", fixtures.stdout)
        self.assertIn("invalid_session=4", fixtures.stdout)
    ...
    def test_ops_certification_policy_cli_matches_checked_in_contract(self):
        ...
        self.assertIn("package_manifest_checksum=0x717b7ebb", generated.stdout)
    ...
    def test_api_abi_policy_cli_matches_checked_in_contract(self):
        ...
        self.assertEqual(fixtures.returncode, 0, fixtures.stderr + fixtures.stdout)
        self.assertIn("status=ok", fixtures.stdout)
        self.assertIn("policy_len=1025", fixtures.stdout)
        self.assertIn("policy_checksum=0x5e460a87", fixtures.stdout)
    ...
    def test_admin_output_schema_cli_matches_checked_in_contract(self):
        ...
        self.assertEqual(fixtures.returncode, 0, fixtures.stderr + fixtures.stdout)
        self.assertIn("status=ok", fixtures.stdout)
        self.assertIn("schema_len=7648", fixtures.stdout)
        self.assertIn("schema_checksum=0xd4f33080", fixtures.stdout)
    ...
    def test_upgrade_rollback_policy_cli_matches_checked_in_contract(self):
        ...
        self.assertEqual(fixtures.returncode, 0, fixtures.stderr + fixtures.stdout)
        self.assertIn("status=ok", fixtures.stdout)
        self.assertIn("policy_len=2144", fixtures.stdout)
        self.assertIn("policy_checksum=0xfd2b541d", fixtures.stdout)
    ...
    def test_durable_catalog_fixtures_cli_validates_storage_root_layout(self):
        ...
        self.assertIn(
            "migration_policy=migrate-legacy-to-v1-reject-future", fixtures.stdout
        )
    ...
    def test_materialize_object_restores_owned_payload_fail_closed(self):
        storage_root = self.root / "materialize-root"
        config_path = self.root / "mem_service.materialize.conf"
        source_path = self.root / "materialize-source.bin"
        destination_path = self.root / "materialized.bin"
        stale_destination = self.root / "materialized-stale.bin"
        bad_checksum_destination = self.root / "materialized-bad-checksum.bin"
        payload = bytes(((i * 13) % 251 for i in range(16391)))
        checksum = _fnv1a64(payload)
        ...
        try:
            ...
            self.assertEqual(materialized.returncode,
                             0,
                             materialized.stderr + materialized.stdout)
            self.assertEqual(destination_path.read_bytes(), payload)

            destination_path.write_bytes(b"caller-owned")
            no_overwrite = self._run_client(
                "materialize-object",
                "--connect",
                f"unix:{self.socket}",
                "--key",
            ...
            self.assertNotEqual(no_overwrite.returncode,
                                0,
                                no_overwrite.stderr + no_overwrite.stdout)
            self.assertIn("status=version_conflict", no_overwrite.stdout)
            self.assertEqual(destination_path.read_bytes(), b"caller-owned")

            stale = self._run_client(
                "materialize-object",
                "--connect",
                f"unix:{self.socket}",
                "--key",
            ...
            self.assertNotEqual(stale.returncode, 0, stale.stderr + stale.stdout)
            self.assertIn("status=stale_ref", stale.stdout)
            self.assertFalse(stale_destination.exists())

            bad_checksum = self._run_client(
                "materialize-object",
                "--connect",
                f"unix:{self.socket}",
                "--key",
                "materialized-object",
                "--to",
                str(bad_checksum_destination),
                "--expected-version",
                "41",
                "--expected-checksum",
                str(checksum + 1),
            )
            self.assertNotEqual(bad_checksum.returncode,
                                0,
                                bad_checksum.stderr + bad_checksum.stdout)
            self.assertIn("status=checksum_mismatch", bad_checksum.stdout)
            self.assertFalse(bad_checksum_destination.exists())

            metrics = self._run_client(
                "metrics",
                "--connect",
                f"unix:{self.socket}",
            )
            self.assertEqual(metrics.returncode, 0, metrics.stderr + metrics.stdout)
            self.assertIn("materialize_object_count=4", metrics.stdout)
        finally:
            self._stop_server(proc)
    ...
    def test_compat_matrix_cli_matches_checked_in_contract(self):
        ...
        self.assertIn("matrix_len=2226", fixtures.stdout)
        self.assertIn("matrix_checksum=0xef0fb6d9", fixtures.stdout)
        self.assertIn("operations=39", fixtures.stdout)
        self.assertIn("fields=229", fixtures.stdout)
    ...
    def test_compat_baseline_cli_matches_checked_in_contract(self):
        ...
        self.assertIn("baseline_len=1252", fixtures.stdout)
        self.assertIn("baseline_checksum=0xa4c8146c", fixtures.stdout)
    ...
    def test_compat_old_new_matrix_cli_matches_checked_in_contract(self):
        ...
        self.assertEqual(fixtures.returncode, 0, fixtures.stderr + fixtures.stdout)
        self.assertIn("status=ok", fixtures.stdout)
        self.assertIn("matrix_len=1734", fixtures.stdout)
        self.assertIn("matrix_checksum=0x6aef243d", fixtures.stdout)
        self.assertIn("old_payloads=39", fixtures.stdout)
        self.assertIn("current_payloads=39", fixtures.stdout)
    ...
    def test_wire_schema_cli_matches_checked_in_contract(self):
        ...
        self.assertEqual(fixtures.returncode, 0, fixtures.stderr + fixtures.stdout)
        self.assertIn("status=ok", fixtures.stdout)
        self.assertIn("manifest_len=17613", fixtures.stdout)
        self.assertIn("manifest_checksum=0xb0964fb0", fixtures.stdout)
        self.assertIn("operations=39", fixtures.stdout)
        self.assertIn("fields=229", fixtures.stdout)
...
class MemServiceReleaseInstallTests(unittest.TestCase):
    def test_print_targets_render_manifests_from_current_binary(self):
        app_dir = ROOT / "apps" / "mem_service"
        targets = {
            "print-release-manifest": app_dir / "release-manifest.txt",
            "print-package-manifest": app_dir / "package-manifest.txt",
            "print-wire-schema": app_dir / "wire-schema.txt",
            "print-admin-output-schema": app_dir / "admin-output-schema.txt",
            "print-upgrade-rollback-policy": app_dir / "upgrade-rollback-policy.txt",
            "print-ops-certification-policy": app_dir / "ops-certification-policy.txt",
            "print-alert-rules": (
                app_dir / "deploy" / "linqu_mem_service.prometheus-alerts.yml"
            ),
            "print-api-abi-policy": app_dir / "api-abi-policy.txt",
            "print-compat-matrix": app_dir / "compat-matrix.txt",
            "print-compat-baseline-v1": app_dir / "compat-baseline-v1.txt",
            "print-compat-old-new-matrix": app_dir / "compat-old-new-matrix.txt",
        }
    ...
    def test_make_install_smoke_creates_release_layout(self):
        ...
        with tempfile.TemporaryDirectory(prefix="msvc_install_", dir=str(_tmp_parent())) as tmp:
            ...
            self.assertIn("package_manifest_checksum=0x717b7ebb", manifest.read_text())
            self.assertIn(
                "installed_sdk_preflight=scripts/verify_mem_service_installed_sdk.sh --preflight",
                manifest.read_text(),
            )
            ...
            self.assertIn("installed_file_count=57", package_manifest.read_text())
            ...
            self.assertIn("wire_schema_manifest_checksum=0xb0964fb0", manifest.read_text())
            self.assertIn("admin_output_schema_checksum=0xd4f33080", manifest.read_text())
            ...
            self.assertIn("upgrade_rollback_policy_checksum=0xfd2b541d", manifest.read_text())
            ...
            self.assertIn("api_abi_policy_checksum=0x5e460a87", manifest.read_text())
            ...
            self.assertIn(
                ...
                manifest.read_text())
            self.assertIn("compat_matrix_checksum=0xef0fb6d9", manifest.read_text())
            self.assertIn("compat_baseline_checksum=0xa4c8146c", manifest.read_text())
            self.assertIn("compat_old_new_matrix_checksum=0x6aef243d",
                          manifest.read_text())
            self.assertIn(
