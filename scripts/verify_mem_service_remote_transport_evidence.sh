...
usage() {
  cat <<'EOF'
Usage: verify_mem_service_remote_transport_evidence.sh --evidence-file PATH [--app-dir DIR] [--dry-run]

Verifies a mem_service cross-host remote transport evidence artifact.

Options:
  --evidence-file PATH  Evidence file produced by run_mem_service_remote_transport_ci.sh.
...
resolve_host_bin() {
  ...
  HOST_BIN="$APP_DIR/linqu_mem_service_host"
  if [[ ! -d "$APP_DIR" ]]; then
    echo "[mem-service-remote-transport-evidence] FAIL: app directory not found: $APP_DIR" >&2
    exit 1
  fi
  if [[ ! -x "$HOST_BIN" ]]; then
    make -C "$APP_DIR" linqu_mem_service_host
...
if [[ -z "$EVIDENCE_FILE" ]]; then
  echo "[mem-service-remote-transport-evidence] FAIL: --evidence-file is required" >&2
  exit 2
fi
...
if [[ ! -f "$EVIDENCE_FILE" ]]; then
  echo "[mem-service-remote-transport-evidence] FAIL: evidence file not found: $EVIDENCE_FILE" >&2
  exit 1
fi

resolve_host_bin
"$HOST_BIN" remote-transport-verify --evidence-file "$EVIDENCE_FILE"
printf '[mem-service-remote-transport-evidence] PASS evidence=%s\n' "$EVIDENCE_FILE"
