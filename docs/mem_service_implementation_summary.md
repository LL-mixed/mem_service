...
> 历史归档说明：本文记录的是从 `ub_sim` 抽取前的实现与验证过程，文中的
> `guest-linux/aarch64/...` 路径不再是独立仓库的当前操作入口。当前构建、安装
> 和下游集成分别以仓库根 `README.md`、`docs/user-manual.md`、
> `docs/integration-ub-sim.md` 和 `docs/integration-ds4.md` 为准。2026-07-01
> 的 certification bundle 未随独立仓库迁移，不能用于证明新仓库当前 revision
> 已认证；新发布必须重新生成并保存可复验 evidence。
...
- **历史生产部署认证曾通过**：2026-07-01 在 `hw-910c:/home/ll/ub_sim` 的 Docker test bed 中完成 Linux ops certification、remote transport certification 和最终 release certification verify，`release-readiness` 当时达到 `certified`；该 evidence 未迁移，不能继承到当前独立仓库 revision。

所以当前更准确的状态是：**独立构建、安装、打包和本地下游契约已经成立；
当前独立仓库 revision 的生产 release certification 仍需重新生成 Linux ops 与
cross-host remote transport evidence 后才能成立。**
...
test bed 的 Linux ops、remote transport 和 release certification，但当前独立
仓库 revision 必须重新生成认证 evidence。W5 Qwen3-0.6B 4-step 与
GSVA-backed prefix/KV reuse 的历史验证保留为迁移基线，后续需要由下游固定
门禁持续复验。
