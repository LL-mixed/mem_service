...
enum mem_service_managed_result mem_service_managed_poll_recovery(
    ...
    uint64_t after_generation,
    struct mem_service_managed_view *view_out);
/* Enumerate quarantined allocations still carrying at least one holder bound
 * to the exact old provider identity on this node. The active replacement
 * provider uses this view to perform local physical revocation before it
 * submits a fencing receipt. */
